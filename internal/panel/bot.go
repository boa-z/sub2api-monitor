...
func (b *Bot) handleCallback(ctx context.Context, cq *telegram.CallbackQuery) error {
	...
	case data == "al:heal_related":
		if b.denyIfNotAdmin(ctx, chatID, msgID, cq.From.ID, cq.ID) {
			return nil
		}
		return b.healRelatedFromAlerts(ctx, chatID, msgID, cq.From.ID)
...
/search &lt;关键词&gt; — 搜索账号
管理 → 实例用户 / 分组 — 搜索、状态/角色筛选与只读详情（热并发/近期错误）
• 运维错误列表：本页样本按平台/用户汇总，可跳转浏览/用户详情
• 主面板/状态/看板会提示流量骤降与实时错误率
...
• <b>管理员</b>：运维视图 + 账号管理写操作（调度/启停/清错/一键修复/临时停调度含自定义时长/启用/重置额度/批量/搜索/错误分页/异常账号/面板用户角色）
