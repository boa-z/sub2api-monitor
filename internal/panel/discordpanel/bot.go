...
func (b *Bot) handleComponent(ctx context.Context, it *discord.Interaction, uid int64, user *discord.User) error {
	...
	case data == "av:heal_related":
		...
		_ = b.respondUpdate(ctx, it, "⏳ 正在修复可用性异常账号…", nil)
		text, comps := b.healRelatedFromAvailability(ctx, uid)
		return b.followupEdit(ctx, it, text, comps)
	case data == "al:heal_related":
		if !b.isAdmin(uid) {
			return b.update(ctx, it, "⛔ 需要管理员权限", b.homeComponents(uid))
		}
		_ = b.respondUpdate(ctx, it, "⏳ 正在修复告警关联账号…", nil)
		text, comps := b.healRelatedFromAlerts(ctx, uid)
		return b.followupEdit(ctx, it, text, comps)
