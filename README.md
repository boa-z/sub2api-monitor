...
| 运维视图（看板/可用性/告警/错误…） | ❌ | ✅ |
| 错误标记已解决（单条/批量） | ❌ | ✅ |
| 账号管理（调度/启停/清错/恢复/刷新/测试） | ❌ | ✅ |
| 临时停调度 / 重置额度 / 用量快照 | ❌ | ✅ |
| 搜索 / 批量清错·恢复·开调度·清限速·一键修复 / 实例用户与分组 | ❌ | ✅ |
| 面板用户角色（列表 / admin·user·清除覆盖） | ❌ | ✅ |
| 错误分标签分页 + 实时页快捷修复 | ❌ | ✅ |
...
5. **提权 / 降权**（运行时）  
   - **面板内（推荐）**：管理员 → 账号管理 → **面板用户** → 选择用户 → 设为管理员 / 用户 / 清除覆盖  
   - 或编辑 `data/users.json` 将对应用户 `role` 设为 `admin` / `user` / 删除字段（继承配置）；`Update` 持久化后立即生效  
...
| **运维视图**（管理员） | 主面板运维快照 + 看板/异常快捷；看板 / 可用性 / 内置告警（触发·已恢复汇总，文案抽账号直达管理）/ 请求与上游错误（分标签+分页，解决后保留页码/批量/直达修复·实时·管理）/ 并发 / 渠道探测（启用·正常·异常汇总、异常优先、7d 可用率）/ 异常账号（error·限速·停调度·汇总分标签+分页，管理/实时/修复/批量/一键监控）；运维菜单含健康摘要；看板一键跳转异常/限速/错误；可用性/并发/告警/渠道上下文跳转；账号管理返回来源记忆；批量操作带进度与失败账号直达管理；各视图可刷新 |
| **账号管理**（管理员） | 健康摘要；浏览（状态/平台/搜索，含停调度·限速筛选）/ 批量清错·恢复·开调度·清限速·一键修复（均需确认）/ 实例用户与分组只读 / **面板用户**（角色 admin/user/清除 + 开关监控/数据源）/ 单账号与实时页一键修复 / 调度开关（二次确认）/ 启停状态 / 测试连通 / 清错误·限速 / 恢复·刷新 / 临时停调度（15m–24h）/ 重置额度 / 用量快照 / 加入监控 |
...
按钮交互：连接、监控账号、阈值；管理员可用运维（看板/可用性/告警/错误分标签分页/并发/渠道/异常账号分标签分页+管理/实时/修复）、账号浏览（状态·平台·停调度·限速筛选 + 下拉选择）、单账号管理（调度/清错/限速/一键修复/临时停调度/重置额度/实时用量）、批量清错·恢复·开调度·清限速·一键修复（确认后执行）等。  
普通用户键盘隐藏运维/管理入口；管理员由 `discord.panel.admin_user_ids` 或 `users.json` 的 `role: admin` 判定（无 Telegram 式 chat_id 回退，需显式配置）。
