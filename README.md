...
| **账号管理**（管理员） | 健康摘要 + **当前筛选徽章**；浏览（状态/平台/搜索/异常汇总/禁用/临时停）/ 批量清错·恢复·开调度·清限速·一键修复·启用·清临时停（均需确认；**优先当前浏览/异常 tab 范围**，按钮标签显示范围）/ 实例用户与分组（搜索/状态·角色·仅偏热筛选/平台筛选/分组并发热度/详情只读） / **面板用户**（角色 admin/viewer/user/清除 + 开关监控/数据源）/ 单账号与**上下文优先**实时页（按异常/限速/停调度优先动作）/ 调度开关（二次确认）/ 启停状态 / 测试连通 / 清错误·限速 / 恢复·刷新 / 临时停调度（15m–24h）/ 重置额度 / 用量快照 / 加入监控 |
