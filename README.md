...
| **运维视图**（管理员） | 看板 / 可用性 / 内置告警 / 请求与上游错误（分标签+分页，可标记已解决/批量/直达修复）/ 并发 / 渠道探测 / 异常账号（管理/批量/一键监控）；各视图可刷新 |
| **账号管理**（管理员） | 浏览（状态/平台/搜索，含停调度·限速筛选）/ 批量清错·恢复·开调度·清限速·一键修复（均需确认）/ 实例用户与分组只读 / **面板用户角色**（admin/user/清除覆盖）/ 单账号与实时页一键修复 / 调度开关（二次确认）/ 启停状态 / 测试连通 / 清错误·限速 / 恢复·刷新 / 临时停调度（15m–24h）/ 重置额度 / 用量快照 / 加入监控 |
