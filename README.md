...
5. **提权 / 降权**（运行时）  
   - 编辑 `data/users.json` 将对应用户 `role` 设为 `admin` 或 `user`，保存后下次请求生效（无需重启也可，取决于进程是否已加载；`Update` 会持久化并立即读到）  
...
| **运维视图**（管理员） | 看板 / 可用性 / 内置告警 / 请求与上游错误（可标记已解决，支持批量）/ 并发 / 渠道探测 / 异常账号（管理/批量/一键监控） |
| **账号管理**（管理员） | 浏览（状态/平台/搜索，含停调度·限速筛选）/ 批量清错·恢复·开调度·清限速·一键修复（均需确认）/ 用户与分组只读 / 单账号一键修复（清错+清限速+恢复+开调度）/ 调度开关（二次确认）/ 启停状态 / 测试连通 / 清错误·限速 / 恢复·刷新 / 临时停调度（15m–24h）/ 重置额度 / 用量快照 / 加入监控；异常账号可直达管理 |
