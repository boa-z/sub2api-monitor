...
| **运维视图**（管理员） | 看板 / 可用性 / 内置告警 / 请求与上游错误（分标签+分页，解决后保留页码/批量/直达修复）/ 并发 / 渠道探测 / 异常账号（error·限速·停调度·汇总分标签，管理/批量/一键监控）；各视图可刷新 |
...
按钮交互：连接、监控账号、阈值；管理员可用运维（看板/可用性/告警/错误分标签分页/并发/渠道/异常账号）、账号浏览（状态·平台·停调度·限速筛选 + 下拉选择）、单账号管理（调度/清错/限速/一键修复/临时停调度/重置额度/实时用量）、批量清错·恢复·开调度·清限速·一键修复（确认后执行）等。  
